...
if [ ! -s "v606vul_ztf_photometry.csv" ];then
...
cat "v606vul_ztf_photometry.csv" | grep ',zg,' | awk -F',' '{if( $3 < 59410.384)print $3" "$4" "$5}' > v606vul_ztf_g_preeruption.txt
cat "v606vul_ztf_photometry.csv" | grep ',zg,' | awk -F',' '{if( $3 > 59670.470)print $3" "$4" "$5}' > v606vul_ztf_g_postdustdip.txt
